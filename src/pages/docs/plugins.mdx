...
插件可以让你注册新的样式，让Tailwind用JavaScript代替CSS注入用户的样式表。

要开始使用你的第一个插件，从`tailwindcss/plugin`中导入Tailwind的`plugin`方法。然后在你的`plugins`数组中，用一个匿名函数作为第一个参数调用它。
...
Plugin里的匿名函数接受一个对象，可以[解构](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Operators/Destructuring_assignment)成多个方法:

- `addUtilities()`, 注册新的@utility样式
- `addComponents()`, 注册新的@component样式
- `addBase()`, 注册新的@base样式
- `addVariant()`, 注册自定义变体
- `e()`,转移类名字符串，以便更安全的时候
- `prefix()`, 手动地将用户配置的前缀应用到选择器的各个部分
- `theme()`, 获取用户在`theme`中的配置
- `variants()`,获取用户在`variants`中的配置
- `config()`, 获取用户在Tailwind的配置
- `postcss`, 直接使用[PostCSS](https://api.postcss.org/postcss.html)进行更底层的操作
